...
std::string HelpMessageCli()
{
    ...
    strUsage += "\n" + _("SSL options: (see the sequence Wiki for SSL setup instructions)") + "\n";
...
static bool AppInitRPC(int argc, char* argv[])
{
    ...
    if (argc<2 || mapArgs.count("-?") || mapArgs.count("-h") || mapArgs.count("-help") || mapArgs.count("-version")) {
        std::string strUsage = _("Sequence RPC client version") + " " + FormatFullVersion() + "\n";
        if (!mapArgs.count("-version")) {
            strUsage += "\n" + _("Usage:") + "\n" +
                  "  sequence-cli [options] <command> [params]  " + _("Send command to Sequence") + "\n" +
                  "  sequence-cli [options] help                " + _("List commands") + "\n" +
                  "  sequence-cli [options] help <command>      " + _("Get help for a command") + "\n";
...
UniValue CallRPC(const string& strMethod, const UniValue& params)
{
    ...
    // sequence:   
    // we get sequenced output (possibly binary) of .write() that either:
