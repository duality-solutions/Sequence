...
void MarkBlockAsInFlight(NodeId nodeid, const uint256& hash, const Consensus::Params& consensusParams, CBlockIndex *pindex = nullptr) {
    ...
    QueuedBlock newentry = {hash, pindex, false, pindex != nullptr};
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck, bool fWriteNames)
{
    ...
    for (unsigned int i = 0; i < block.vtx.size(); i++)
    {
        ...
        else
        {
            ...
            if (pindex->nHeight > trustdepth && !CheckInputs(tx, state, view, fScriptChecks, flags, false, nScriptCheckThreads ? &vChecks : nullptr))
                return false;
...
bool static DisconnectTip(CValidationState &state, const Consensus::Params& consensusParams) {
    ...
    {
        CCoinsViewCache view(pcoinsTip);
        if (!DisconnectBlock(block, state, pindexDelete, view, nullptr, true))
...
static bool IsSuperMajority(int minVersion, const CBlockIndex* pstart, unsigned nRequired, const Consensus::Params& consensusParams)
{
    ...
    for (int i = 0; i < Params().GetConsensus().nToCheckBlockUpgradeMajority && nFound < nRequired && pstart != nullptr; i++)
    {
