...
UniValue getnewaddress(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() > 1)
        throw std::runtime_error(
            ...
            "\nReturns a new Sequence address for receiving payments.\n"
            "If 'account' is specified (recommended), it is added to the address book \n"
            "so payments received with the address will be credited to 'account'.\n"
            "\nArguments:\n"
            "1. \"account\"        (string, optional) The account name for the address to be linked to. if not provided, the default account \"\" is used. It can also be set to the empty string \"\" to represent the default account. The account does not need to exist, it will be created if there is no account by the given name.\n"
            "\nResult:\n"
            "\"sequenceaddress\"    (string) The new Sequence address\n"
            "\nExamples:\n"
...
UniValue getaccountaddress(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw std::runtime_error(
            ...
            "\nReturns the current Sequence address for receiving payments to this account.\n"
            "\nArguments:\n"
            "1. \"account\"       (string, required) The account name for the address. It can also be set to the empty string \"\" to represent the default account. The account does not need to exist, it will be created and a new address created  if there is no account by the given name.\n"
            "\nResult:\n"
            "\"sequenceaddress\"   (string) The account Sequence address\n"
            "\nExamples:\n"
...
UniValue getrawchangeaddress(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() > 1)
        throw std::runtime_error(
            "getrawchangeaddress\n"
            "\nReturns a new Sequence address, for receiving change.\n"
...
UniValue setaccount(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw std::runtime_error(
            "setaccount \"sequenceaddress\" \"account\"\n"
            "\nSets the account associated with the given address.\n"
            "\nArguments:\n"
            "1. \"sequenceaddress\"  (string, required) The Sequence address to be associated with an account.\n"
...
UniValue getaccount(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw std::runtime_error(
            "getaccount \"sequenceaddress\"\n"
            "\nReturns the account associated with the given address.\n"
            "\nArguments:\n"
            "1. \"sequenceaddress\"  (string, required) The Sequence address for account lookup.\n"
            "\nResult:\n"
...
UniValue getaddressesbyaccount(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw std::runtime_error(
            ...
            "  \"sequenceaddress\"  (string) a Sequence address associated with the given account\n"
            "  ,...\n"
            "]\n"
            "\nExamples:\n"
...
UniValue sendtoaddress(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 4)
        throw std::runtime_error(
            "sendtoaddress \"sequenceaddress\" amount ( \"comment\" \"comment-to\" )\n"
            "\nSend an amount to a given address. The amount is a real and is rounded to the nearest 0.00000001\n"
            + HelpRequiringPassphrase() +
            "\nArguments:\n"
            "1. \"sequenceaddress\"  (string, required) The Sequence address to send to.\n"
            "2. \"amount\"      (numeric, required) The amount in Sequence to send. eg 0.1\n"
...
UniValue listaddressgroupings(const UniValue& params, bool fHelp)
{
    if (fHelp)
        throw std::runtime_error(
            ...
            "  [\n"
            "    [\n"
            "      \"sequenceaddress\",     (string) The Sequence address\n"
            "      amount,                 (numeric) The amount in Sequence\n"
...
UniValue signmessage(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 2)
        throw std::runtime_error(
            "signmessage \"sequenceaddress\" \"message\"\n"
            "\nSign a message with the private key of an address"
            + HelpRequiringPassphrase() + "\n"
            "\nArguments:\n"
            "1. \"sequenceaddress\"  (string, required) The Sequence address to use for the private key.\n"
...
UniValue getreceivedbyaddress(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw std::runtime_error(
            "getreceivedbyaddress \"sequenceaddress\" ( minconf )\n"
            "\nReturns the total amount received by the given sequenceaddress in transactions with at least minconf confirmations.\n"
            "\nArguments:\n"
            "1. \"sequenceaddress\"  (string, required) The Sequence address for transactions.\n"
            "2. minconf             (numeric, optional, default=1) Only include transactions confirmed at least this many times.\n"
            "\nResult:\n"
            "amount   (numeric) The total amount in Sequence received at this address.\n"
            "\nExamples:\n"
...
UniValue getreceivedbyaccount(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw std::runtime_error(
            ...
            "2. minconf          (numeric, optional, default=1) Only include transactions confirmed at least this many times.\n"
            "\nResult:\n"
            "amount              (numeric) The total amount in Sequence received for this account.\n"
            "\nExamples:\n"
...
UniValue movecmd(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 3 || params.size() > 5)
        throw std::runtime_error(
            ...
            "\nExamples:\n"
            "\nMove 0.01 Sequence from the default account to the account named tabby\n"
            + HelpExampleCli("move", "\"\" \"tabby\" 0.01") +
            "\nMove 0.01 Sequence timotei to akiko with a comment and funds have 10 confirmations\n"
...
UniValue sendfrom(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 3 || params.size() > 6)
        throw std::runtime_error(
            "sendfrom \"fromaccount\" \"tosequenceaddress\" amount ( minconf \"comment\" \"comment-to\" )\n"
            "\nSent an amount from an account to a Sequence address.\n"
            "The amount is a real and is rounded to the nearest 0.00000001."
            + HelpRequiringPassphrase() + "\n"
            "\nArguments:\n"
            "1. \"fromaccount\"       (string, required) The name of the account to send funds from. May be the default account using \"\".\n"
            "2. \"tosequenceaddress\"  (string, required) The Sequence address to send funds to.\n"
            "3. amount                (numeric, required) The amount in Sequence. (transaction fee is added on top).\n"
            ...
            "\"transactionid\"        (string) The transaction id.\n"
            "\nExamples:\n"
            "\nSend 0.01 Sequence from the default account to the address, must have at least 1 confirmation\n"
...
UniValue sendmany(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 4)
        throw std::runtime_error(
            ...
            "    {\n"
            "      \"address\":amount   (numeric) The Sequence address is the key, the numeric amount in Sequence is the value\n"
            "      ,...\n"
            "    }\n"
...
UniValue listreceivedbyaddress(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() > 3)
        throw std::runtime_error(
            ...
            "    \"amount\" : x.xxx,                  (numeric) The total amount in Sequence received by the address\n"
...
UniValue listtransactions(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() > 4)
        throw std::runtime_error(
            ...
            "    \"address\":\"sequenceaddress\",    (string) The Sequence address of the transaction. Not present for \n"
            "                                                move transactions (category = move).\n"
            "    \"category\":\"send|receive|move\", (string) The transaction category. 'move' is a local (off blockchain)\n"
            "                                                transaction between accounts, and not associated with an address,\n"
            "                                                transaction id or block. 'send' and 'receive' transactions are \n"
            "                                                associated with an address, transaction id and block details\n"
            "    \"amount\": x.xxx,          (numeric) The amount in Sequence. This is negative for the 'send' category, and for the\n"
            "                                         'move' category for moves outbound. It is positive for the 'receive' category,\n"
            "                                         and for the 'move' category for inbound funds.\n"
            "    \"vout\" : n,               (numeric) the vout value\n"
            "    \"fee\": x.xxx,             (numeric) The amount of the fee in Sequence. This is negative and only available for the \n"
...
UniValue listsinceblock(const UniValue& params, bool fHelp)
{
    if (fHelp)
        throw std::runtime_error(
            ...
            "    \"address\":\"sequenceaddress\",    (string) The Sequence address of the transaction. Not present for move transactions (category = move).\n"
            "    \"category\":\"send|receive\",     (string) The transaction category. 'send' has negative amounts, 'receive' has positive amounts.\n"
            "    \"amount\": x.xxx,          (numeric) The amount in Sequence. This is negative for the 'send' category, and for the 'move' category for moves \n"
            "                                          outbound. It is positive for the 'receive' category, and for the 'move' category for inbound funds.\n"
            "    \"vout\" : n,               (numeric) the vout value\n"
            "    \"fee\": x.xxx,             (numeric) The amount of the fee in Sequence. This is negative and only available for the 'send' category of transactions.\n"
...
UniValue gettransaction(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw std::runtime_error(
            ...
            "\nResult:\n"
            "{\n"
            "  \"amount\" : x.xxx,        (numeric) The transaction amount in Sequence\n"
            ...
            "      \"address\" : \"sequenceaddress\",   (string) The Sequence address involved in the transaction\n"
            "      \"category\" : \"send|receive\",    (string) The category, either 'send' or 'receive'\n"
            "      \"amount\" : x.xxx                  (numeric) The amount in Sequence\n"
            "      \"vout\" : n,                       (numeric) the vout value\n"
            "    }\n"
            "    ,...\n"
            "  ],\n"
...
UniValue walletpassphrase(const UniValue& params, bool fHelp)
{
    if (pwalletMain->IsCrypted() && (fHelp || params.size() < 2 || params.size() > 3))
        throw std::runtime_error(
            ...
            "This is needed prior to performing transactions related to private keys such as sending sequences\n"
            "\nArguments:\n"
...
UniValue encryptwallet(const UniValue& params, bool fHelp)
{
    if (!pwalletMain->IsCrypted() && (fHelp || params.size() != 1))
        throw std::runtime_error(
            ...
            "\nNow set the passphrase to use the wallet, such as for signing or sending sequence\n"
            + HelpExampleCli("walletpassphrase", "\"my pass phrase\"") +
            "\nNow we can so something like sign\n"
            + HelpExampleCli("signmessage", "\"sequenceaddress\" \"test message\"") +
...
UniValue listunspent(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() > 3)
        throw std::runtime_error(
            ...
            "3. \"addresses\"    (string) A json array of sequence addresses to filter\n"
            "    [\n"
            ...
            "    \"vout\" : n,               (numeric) the vout value\n"
            "    \"address\" : \"address\",  (string) the sequence address\n"
    ...
    if (params.size() > 2) {
        ...
        for (unsigned int idx = 0; idx < inputs.size(); idx++) {
            ...
            if (!address.IsValid())
                throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, std::string("Invalid Sequence address: ")+input.get_str());
            if (setAddress.count(address))
...
UniValue lockunspent(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw std::runtime_error(
            ...
            "A locked transaction output will not be chosen by automatic coin selection, when spending sequences.\n"
...
UniValue settxfee(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 1 || AmountFromValue(params[0]) < MIN_TX_FEE)
        throw std::runtime_error(
            ...
            "\nArguments:\n"
            "1. amount         (numeric, required) The transaction fee in SEQ/kB rounded to the nearest 0.00000001\n"
            "\nResult\n"
...
UniValue printAddresses(const UniValue& params)
{
    ...
    for (const COutput& out : vCoins)
    {
        ...
        std::string strAdd = CSequenceAddress(utxoAddress).ToString();
...
UniValue multisend(const UniValue& params, bool fHelp)
{
    ...
    if(params.size() == 1)
    {
        ...
        else if (strCommand == "enable" || strCommand == "activate" )
        {
            if(pwalletMain->vMultiSend.size() < 1)
                throw JSONRPCError(RPC_INVALID_REQUEST, "Unable to activate MultiSend, check MultiSend List");
            if(CSequenceAddress(pwalletMain->vMultiSend[0].first).IsValid())
            {
                ...
                throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Unable to activate MultiSend, check MultiSend List");
        }
        ...
        else if(strCommand == "enableall")
        {
           pwalletMain->vDisabledAddresses.clear();
            ...
            if(!walletdb.EraseMSDisabledAddresses(pwalletMain->vDisabledAddresses))
                return "failed to clear old list from walletDB"; // No conversion for this return string
            else
            {
                pwalletMain->vDisabledAddresses.clear();
                return printMultiSend(params);
    ...
    if(params.size() == 2 && params[0].get_str() == "delete")
    {
        ...
        if(!walletdb.EraseMultiSend(pwalletMain->vMultiSend))
            throw JSONRPCError(RPC_DATABASE_ERROR, "failed to delete old MultiSend List from database");
    ...
    if(params.size() == 2 && params[0].get_str() == "disable")
    {
        ...
        if(!CSequenceAddress(disAddress).IsValid())
        ...
        else
        {
            ...
            if(!walletdb.EraseMSDisabledAddresses(pwalletMain->vDisabledAddresses))
                throw JSONRPCError(RPC_DATABASE_ERROR, "disabled address from sending, but failed to clear old list from walletDB");
    ...
    if (fHelp || params.size() != 2)
        throw std::runtime_error(
            ...
            "   print - displays the current MultiSend List \n"
            "   clear - deletes the current MultiSend List \n"
            "   enable/activate - activates the current MultiSend List \n"
            "   disable/deactivate - disables the current MultiSend List \n"
            "   delete <Address #> - deletes an address from the MultiSend List \n"
            "   disable <address> - prevents a specific address from sending MultiSend transactions\n"
            "   enableall - enables all addresses to be eligible to send MultiSend transactions\n"
            
            "****************************************************************\n"
            "TO CREATE OR ADD TO THE MULTISEND LIST:\n"
            "multisend <Sequence Address> <percent>\n"
            "This will add a new address to the MultiSend List\n"
    ...
    LOCK(pwalletMain->cs_wallet);
    {
        ...
        if (nPercent + sumMultiSend() > 100)
            throw JSONRPCError(RPC_INVALID_PARAMETER, "Failed to add to MultiSend List, the sum of your MultiSend is greater than 100%");

        for(unsigned int i = 0; i < pwalletMain->vMultiSend.size(); i++)
        {
            if(pwalletMain->vMultiSend[i].first == strAddress)
                throw JSONRPCError(RPC_INVALID_PARAMETER, "Failed to add to MultiSend List, cannot use the same address twice");
        }
